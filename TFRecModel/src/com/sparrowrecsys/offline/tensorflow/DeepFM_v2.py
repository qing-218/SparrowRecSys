...
# 自定义ReduceLayer层，用于对输入进行聚合操作（可以选择sum或mean）
class ReduceLayer(tf.keras.layers.Layer):
    def __init__(self, axis, op='sum', **kwargs):
        super().__init__()
        self.axis = axis
        self.op = op
        assert self.op in ['sum', 'mean'] # 限制操作类型只能是 'sum' 或 'mean'
    def build(self, input_shape):
        pass

    def call(self, input, **kwargs):
        if self.op == 'sum':
            return tf.reduce_sum(input, axis=self.axis)  # 沿指定轴求和
        elif self.op == 'mean':
            return tf.reduce_mean(input, axis=self.axis)# 沿指定轴求均值
        return tf.reduce_sum(input, axis=self.axis)# 默认为求和操作
